...
class TimelapseDatasetNDC(torch.utils.data.Dataset):
    """
...
def convert_from_homogenous_to_ndc(pose, z_near, z_far, width, height):
    Xv, Yv, Zv, Nv, Fv, Wvp, Hvp = pose[3, 0], pose[3, 1], pose[3, 2], z_near, z_far, width, height
    Xc = Zv*Xv / (Wvp / 2) #Convert to clip space
    Yc = Zv*Yv / (Hvp / 2)
    Zc = (Zv - Nv)/(Fv + Nv) + Nv
    Wc = Zv
