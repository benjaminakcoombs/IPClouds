...
class PosedImagesDataset(torch_data.Dataset):
    def __init__(
        ...
        self._cached_images, self._cached_poses, self._cached_masks = None, None, None
        cache_device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        try:
            # attempt caching of the data in gpu-memory first
            self._cached_images, self._cached_poses, self._cached_masks = self._cache_all_data(
    ...
    def _cache_all_data(
        ...
        images_cache, poses_cache, masks_cache = {}, {}, {}
        for image_file_path in self._image_file_paths:
            i = -13
            if str(image_file_path)[-13:-8] == "train":
                i = -14
            image_mask_path = Path(os.path.join(str(image_file_path)[:i], "mask", str(image_file_path)[-7:]))
...
#            image = image.resize((60, 80))
            ...
            mask = self._process_image(mask).to(cache_device)
            masks_cache[image_file_path] = mask
        ...
        return images_cache, poses_cache, masks_cache
    ...
    def get_hemispherical_radius_estimate(self) -> float:
        ...
        return hemispherical_radius_estimate * 2
    ...
    def __getitem__(self, index: int) -> Tuple[Tensor, Tensor]:
        ...
        if self._cached_data_mode:
            ...
            i = -13
            if image_file_path[-13:-8] == "train":
                i = -14
            image_mask_path = os.path.join(image_file_path[:i], "mask", image_file_path[-7:])
            mask = Image.open(image_mask_path)
        ...
        if self._image_data_range != default_image_range:
            ...
            mask = adjust_dynamic_range(
                mask, drange_in=default_image_range, drange_out=self._image_data_range
            )

        # return the image and it's camera pose
        return image, unified_pose, mask
