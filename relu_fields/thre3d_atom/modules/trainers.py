...
def train_sh_vox_grid_vol_mod_with_posed_images(
    ...
    if render_feedback_pose is None:
        ...
        render_feedback_pose = CameraPose(
            rotation=feedback_dataset[0][-2][:, :3].cpu().numpy(),
            translation=feedback_dataset[0][-2][:, 3:].cpu().numpy(),
        )
        real_feedback_image = feedback_dataset[0][0].permute(1, 2, 0).cpu().numpy()
    image_batch_cache_size = len(train_dataset)
    ...
    for stage in range(1, num_stages + 1):
        ...
        train_dl = _make_dataloader_from_dataset(
            current_stage_train_dataset, image_batch_cache_size, num_workers #4 images; 8; 4
        )
        ...
        for stage_iteration in range(1, num_iterations_per_stage + 1):
            ...
            images, poses, masks = next(infinite_train_dl)
            ...
            for i in range(len(poses)):
                casted_rays = flatten_rays(
                    cast_rays(
                        ...
                        CameraPose(rotation=poses[i][:, :3], translation=poses[i][:, 3:]),
            ...
            for i in range(len(poses)):
                ...
                rays_list_uni.append(casted_rays)
            rays_uni = collate_rays(rays_list)
            ...
            rays_batch_uni, pixels_batch_uni = sample_random_rays_and_pixels_synchronously(
                rays_uni, pixels, ray_batch_size
            )
            ...
            apply_diffuse_render_regularization = True
            ...
            sparsity_rendered_batch = vol_mod.render_rays(rays_batch_uni)
            sparsity_npoints = 10000
            sparsity_radius = 600
            sp_points = torch.tensor(np.random.uniform(-sparsity_radius, sparsity_radius, size=(sparsity_npoints, 3)), dtype=torch.float64, device=vol_mod.device)

            forward_result = vol_mod.thre3d_repr.forward(sp_points)
            densities = forward_result[..., -1:] #return only the density for the forward result
            sparsity_weight =  2
            sparsity_length = 10   #* 0.05
            if sparsity_weight > 0.0:
                loss_sp = sparsity_weight * (1.0 - torch.exp(- sparsity_length * densities)).mean()
            else:
                loss_sp = 0.0
            log.info("Metrics")
            log.info(loss_sp)
            log.info(diffuse_loss)
            log.info(total_loss)
            total_loss = (total_loss + diffuse_loss) + loss_sp
            log.info(total_loss)
