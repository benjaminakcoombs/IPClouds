...
class VoxelGrid(Module):
    ...
    def _un_normalize_points(self, points: Tensor) -> Tensor:
        normalized_points = torch.empty_like(points, device=points.device)
        for coordinate_index, coordinate_range in enumerate(self._aabb):
            normalized_points[:, coordinate_index] = adjust_dynamic_range(
                points[:, coordinate_index],
                drange_in=(-1.0, 1.0),
                drange_out=coordinate_range,
                slack=True,
            )
        return normalized_points
    ...
    def forward(self, points: Tensor, viewdirs: Optional[Tensor] = None) -> Tensor:
        ...
        normalized_points = self._normalize_points(points).type(dtype=torch.float32)
    ...
    def forward_unnorm(self, points: Tensor, viewdirs: Optional[Tensor] = None) -> Tensor:
        """
        computes the features/radiance at the requested 3D points
        Args:
            points: Tensor of shape [N x 3 (NUM_COORD_DIMENSIONS)]
        ...
        """
        # obtain the range-normalized points for interpolation
        normalized_points = points.type(dtype=torch.float32)#self._normalize_points(points).type(dtype=torch.float32)

        # interpolate and compute densities
        # Note the pre- and post-activations :)
        preactivated_densities = self._un_normalize_points(self._density_preactivation(
            self._densities * self._expected_density_scale
        ))  # note the use of the expected density scale
        log.info("POints are:")
        log.info(preactivated_densities[None, ...].permute(0, 4, 3, 2, 1))
        log.info("UNORM points are:")
        log.info(normalized_points[None, None, None, ...])
        log.info("points end:")
        ...
        interpolated_densities = self._density_postactivation(interpolated_densities)
#        interpolated_densities = self._un_normalize_points(interpolated_densities).type(dtype=torch.float32)
        log.info("ID stuff")
        log.info(interpolated_densities)
